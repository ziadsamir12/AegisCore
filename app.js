...
class AegisCore {
    ...
    initializeToast() {
        try {
            this.notificationToast = new bootstrap.Toast(
    ...
    connectToMQTT() {
        if (typeof mqtt === "undefined") {
            ...
            this.showNotification("MQTT library not loaded. Check internet.", "danger");
            return;
        }

        console.log("Connecting to MQTT:", MQTT_SERVER);
    ...
    handleMessage(topic, payload) {
        ...
        // Ignore non-JSON (like "ESP32 ONLINE")
        if (!payload || payload[0] !== "{") {
            console.warn("Ignoring non-JSON payload on status topic:", payload);
            return;
        }
        ...
        // Now we have a clean status object from ESP
    ...
    updateValve(type, isClosed) {
        ...
        if (type === "water") {
            this.flags.waterValveClosed = !!isClosed;
        } else if (type === "gas") {
            this.flags.gasValveClosed = !!isClosed;
        }
    ...
    setupEventListeners() {
        ...
        if (enableWaterSensorBtn) {
            enableWaterSensorBtn.addEventListener("click", () => {
                this.publishCommand("ENABLE_WATER_SENSOR");
            });
        }
        if (disableWaterSensorBtn) {
            disableWaterSensorBtn.addEventListener("click", () => {
        ...
        if (enableFlameSensorBtn) {
            enableFlameSensorBtn.addEventListener("click", () => {
                this.publishCommand("ENABLE_FLAME_SENSOR");
            });
        }
        if (disableFlameSensorBtn) {
            disableFlameSensorBtn.addEventListener("click", () => {
        ...
        if (enableGasSensorBtn) {
            enableGasSensorBtn.addEventListener("click", () => {
                this.publishCommand("ENABLE_GAS_SENSOR");
            });
        }
        if (disableGasSensorBtn) {
            disableGasSensorBtn.addEventListener("click", () => {
                if (this.flags.gasValveClosed === false) {
        ...
        if (dateInput) {
            dateInput.addEventListener("change", () => this.refreshAnalytics());
        }
        if (dataTypeSelector) {
            dataTypeSelector.addEventListener("change", () => this.refreshAnalytics());
        }
    }
    ...
    updateCharts(dayData) {
        ...
        if (this.charts.water) {
            ...
            this.charts.water.data.datasets[0].data = dayData.map(d => d.water);
            this.charts.water.update();
        }

        if (this.charts.flame) {
            this.charts.flame.data.labels = labels;
            this.charts.flame.data.datasets[0].data = dayData.map(d => d.flame);
            this.charts.flame.update();
        }

        if (this.charts.gas) {
            this.charts.gas.data.labels = labels;
            this.charts.gas.data.datasets[0].data = dayData.map(d => d.gas);
            this.charts.gas.update();
        }

        if (this.charts.temp) {
            this.charts.temp.data.labels = labels;
            this.charts.temp.data.datasets[0].data = dayData.map(d => d.temperature);
            this.charts.temp.update();
        }

        if (this.charts.motion) {
            this.charts.motion.data.labels = labels;
            this.charts.motion.data.datasets[0].data = dayData.map(d => d.motion);
